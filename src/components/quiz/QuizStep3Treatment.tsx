...
export default function QuizStep3Treatment() {
  const { answers, updateAnswers, nextStep, prevStep } = useQuizStore();
  const [selected, setSelected] = useState(answers.currentTreatment || '');
  const [error, setError] = useState('');

  const handleNext = () => {
    if (!selected) { setError('Please select an option.'); return; }
    updateAnswers({ currentTreatment: selected });
    nextStep();
  };
  ...
  return (
    <div>
      ...
      <p className="text-sm mb-5" style={{ color: '#5C5C56' }}>If any</p>
